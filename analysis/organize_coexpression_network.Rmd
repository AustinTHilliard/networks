...
ls()
# if there is only one network block, re-save the dendrogram and delete blockGenes
if (length(blockGenes) == 1) {
  dendro = dendro[[1]]
  rm(blockGenes)
} else {
  # otherwise look at the head of each blockGenes element
  lapply(blockGenes,head)
}

# look at the head of each dendrogram element
dendro
lapply(dendro, function(d) lapply(d, head))

# colors holds the module assignment for each gene (column) in DATA
length(colors)
sort(table(colors))

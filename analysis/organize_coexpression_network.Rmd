...
```{r start clean and source functions, message=FALSE}
rm(list=ls())
...
#### Load expression and network data 
```{r load data}
...
`kME` is a data frame where each row is a gene and the columns represent the correlations (with p-values) to `MEs`. `nrow(kMEs)` should equal `ncol(DATA)` (although genes may not be in same order) and `ncol(kMEs)` should equal `2*ncol(MEs)`.
``` {r kME}
dim(kME)
kME[1:5, 1:4]
```
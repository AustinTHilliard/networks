...
genedfFactorCols
# convert just these columns to characters
genedf[, genedfFactorCols] = as.character(genedf[, genedfFactorCols])
...
This should be the . Here we will use the androgen receptor gene _ar_.
```{r}
xgene = "ar"
```
...
xgeneRow
```
Here are multiple equivalent ways to access the row in `genedf` containing information for the gene name stored in `xgene`.
```{r}
